...
from apps.gateway.routing.engine import RoutingEngine, RoutingRejectedError
...
def test_evaluate_rules_matches_highest_priority_first():
    rules = [
        RuleSpec(
            name="low", condition_expression="latency > 100ms", action_type=RuleActionType.FALLBACK, action_provider="groq", priority=200
        ),
        RuleSpec(
            name="high", condition_expression="latency > 100ms", action_type=RuleActionType.USE, action_provider="ollama", priority=10
        ),
    ]
...
def test_evaluate_rules_skips_disabled_rules():
    rules = [
        RuleSpec(name="disabled", condition_expression="latency > 100ms", action_type=RuleActionType.REJECT, enabled=False, priority=1),
    ]
...
def test_evaluate_rules_skips_unparseable_rule_without_crashing():
    rules = [
        RuleSpec(name="broken", condition_expression="not a valid condition!!", action_type=RuleActionType.REJECT, priority=1),
...
def test_rule_use_action_forces_named_provider_outside_normal_tier():
    ...
    rules = [RuleSpec(name="force-groq", condition_expression="latency > -1ms", action_type=RuleActionType.USE, action_provider="groq")]
...
def test_rule_fallback_action_used_when_primary_unavailable():
    ...
    rules = [
        RuleSpec(
            name="reroute-on-outage",
            condition_expression="provider == unavailable",
            action_type=RuleActionType.FALLBACK,
...
def test_rule_that_does_not_match_falls_through_to_normal_policy():
    engine = _make_engine()
    rules = [RuleSpec(name="never-matches", condition_expression="latency > 999999ms", action_type=RuleActionType.REJECT)]
    decision = engine.route("gpt-4o", rules=rules)
...
def test_rule_naming_disabled_provider_degrades_to_policy_ranking():
    ...
    rules = [RuleSpec(name="force-disabled", condition_expression="latency > -1ms", action_type=RuleActionType.USE, action_provider="groq")]
...
async def test_routing_rules_crud_lifecycle():
    ...
    list_resp = client.get("/routing-rules", params={"organization_id": org_id}, headers=headers)
    assert list_resp.status_code == 200
    assert len(list_resp.json()) == 1

    get_resp = client.get(f"/routing-rules/{rule_id}", headers=headers)
    assert get_resp.status_code == 200

    patch_resp = client.patch(f"/routing-rules/{rule_id}", json={"enabled": False, "priority": 5}, headers=headers)
